...
class UnexpectedEmbeddingError(Exception):
    pass
...
@pytest.mark.asyncio
async def test_unexpected_error_marks_failed_and_reraises(db_session, settings):
    user = User(email="unexpected@example.com", hashed_password="x")
    db_session.add(user)
    await db_session.commit()
    await db_session.refresh(user)
    repo = DocumentIngestionRepository(db_session)
    ...
    service = IngestionService(
        ...
        settings=settings,
        storage=FakeStorage(data=b"hello world"),
        embedding_provider=FailingEmbeddingProvider(UnexpectedEmbeddingError("boom")),
        vector_store=RecordingVectorStore(),
    )

    with pytest.raises(UnexpectedEmbeddingError):
        await service.ingest_document(doc.id)
