...
async def fixture(db_session):
    ...
    conv = Conversation(user_id=user.id, document_id=doc.id, active_document_ids=[str(doc.id)])
    db_session.add(conv)
    await db_session.commit()
...
async def test_success_stream_order_and_persistence(db_session, settings):
    ...
    events = [
        event
        async for event in pipeline.answer(user=user, conversation_id=conv.id, content="Refunds?")
    ]
    ...
    assert {m.role for m in rows} == {MessageRole.USER.value, MessageRole.ASSISTANT.value}
...
async def test_no_context_skips_llm_and_persists_abstention(db_session, settings):
    ...
    pipeline = AnswerPipeline(
        db_session,
        settings=settings,
        retriever=FakeRetriever(RetrievalResult([], NoContextReason.NO_ACTIVE_DOCUMENTS)),
        llm=llm,
    )
