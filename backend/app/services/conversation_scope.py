...
class ConversationScopeService:
    ...
    async def create_conversation(self, *, user: User, document_id: UUID | None, active_document_ids: list[UUID]) -> Conversation:
    ...
    async def update_active_documents(self, *, user: User, conversation_id: UUID, active_document_ids: list[UUID]) -> Conversation:
    ...
    async def get_queryable_scope(self, *, user: User, conversation_id: UUID) -> QueryableConversationScope:
        conv = await self._get_owned_conversation(user=user, conversation_id=conversation_id)
        ...
        rows = list(
            (await self.session.execute(
                select(Document).where(Document.user_id == user.id, Document.id.in_(active_ids))
            )).scalars().all()
        )
    ...
    async def _get_owned_conversation(self, *, user: User, conversation_id: UUID) -> Conversation:
        conv = (await self.session.execute(select(Conversation).where(Conversation.id == conversation_id))).scalar_one_or_none()
        if conv is None:
            raise NotFoundError
    ...
    async def _validate_documents(self, *, user: User, document_ids: list[UUID], require_ready: bool) -> None:
        if not document_ids:
            return
        docs = list(
            (await self.session.execute(select(Document).where(Document.id.in_(document_ids)))).scalars().all()
        )
