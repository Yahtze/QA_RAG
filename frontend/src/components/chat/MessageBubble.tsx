    ...
    if (enableLinks && match) {
      return (
        <button
          key={`${chunk}-${index}`}
          type="button"
          onClick={() => onActivateCitation(match[1]!)}
...
export function MessageBubble({ message, onRetry, onActivateCitation }: { message: Message; onRetry: () => void; onActivateCitation: (label: string) => void }) {
